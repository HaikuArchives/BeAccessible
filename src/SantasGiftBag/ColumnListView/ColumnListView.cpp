...
BetterScrollView(NULL,target,resizingMode,flags,horizontal,vertical,scroll_view_corner,border)
{
	...
	// HACK: In TableWindow, we add the RowSummaryView first, then the ContainerView,
	// and then we resize the ContainerView's horizontal scrollbar to make room for
	// the RowSummaryView.  For reasons beyond my meager understanding, the
	// RowSummaryView's background isn't kept; instead the background of the ContainerView
	// takes priority, so we're setting the background here, which in reality is going
	// to be the RowSummaryView's background.  My head hurts. 
...
fDownArrow(BRect(0.0,0.0,10.0,10.0),B_COLOR_8_BIT,CLVDownArrowData,false,false)
{
	...
	fColumnLabelView = new CLVColumnLabelView(BRect(Frame.left,Frame.top,Frame.right,ColumnLabelViewBottom),this,LabelFont);
	fTextColor = Black;
	fBgColor = White;
...
void ColumnListView::SetSortKey(int32 ColumnIndex)
{
	...
	if(ColumnIndex >= 0)
	{
		...
		if ((Column == NULL)||(!(Column->Flags()&CLV_SORT_KEYABLE))) return;  // bugfix jaf
	}
	else Column = NULL;
	if(fSortKeyList.ItemAt(0) != Column || Column == NULL)
	{
		...
		if(Column)
		{
			...
			if(Column->fSortMode == NoSort) SetSortMode(ColumnIndex,Ascending);
			SortItems();
			fColumnLabelView->Invalidate(BRect(Column->fColumnBegin,LabelBounds.top,Column->fColumnEnd, LabelBounds.bottom));
		}
	}
...
void ColumnListView::SetSorting(int32 NumberOfKeys, int32* SortKeys, CLVSortMode* SortModes)
{
	...
	for(int32 Counter = 0; Counter < NumberOfKeys; Counter++)
	{
		if(Counter == 0) SetSortKey(SortKeys[0]);
		            else AddSortKey(SortKeys[Counter]);
...
BList* ColumnListView::SortItemsInThisLevel(int32 OriginalListStartIndex)
{
	...
	for(Counter = 0; Counter < ItemsInThisLevel; Counter++) SortArray[Counter] = ListItems[Counter];
	SortListArray(SortArray,ItemsInThisLevel);
	for(Counter = 0; Counter < ItemsInThisLevel; Counter++) ListItems[Counter] = SortArray[Counter];
	delete[] SortArray;
	return ThisLevelItems;
}

int ColumnListView :: QuickSortCompareItems(const CLVListItem * item1, const CLVListItem * item2) const
{
	int32 SortDepth = fSortKeyList.CountItems();
	for (int32 SortIteration = 0; SortIteration < SortDepth; SortIteration++)
	{
...
void ColumnListView::SortListArray(CLVListItem** SortArray, int32 n)
{
	if (fCompare) qsort(SortArray, n, sizeof(CLVListItem *), QuickSortCompareItemsFunc);
}
